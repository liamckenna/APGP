...
Scene::Scene(const std::string& filepath, Program& program) : scene_ecs(), resource_manager(), program(program) {
	...
	if (!program.hardcoded) {
		...
		scene_ecs.AddComponent(floor_entity, TransformComponent{ .position = glm::vec3(0.f, 3.f, 0.f), .scale = glm::vec3(0.5f)});
		...
		scene_ecs.AddComponent(surface_entity, TransformComponent{ .position = glm::vec3(8.f, 0.f, 1.f), .scale = glm::vec3(20) });
		...
		scene_ecs.AddComponent(other_floor_entity, TransformComponent{ .position = glm::vec3(0.f, 6.f, 0.f), .scale = glm::vec3(0.1f) });
